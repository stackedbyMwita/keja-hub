...
// GET — fetch current system config
...
export async function POST(req: Request) {
  try {
    ...
    const update = {
      ...
      maintenance_message:           String(body.maintenance_message ?? '').trim() || 'KéjaLink is currently undergoing scheduled maintenance. We will be back shortly.',
      disable_signups:               Boolean(body.disable_signups),
      disable_signups_message:       String(body.disable_signups_message ?? '').trim() || 'New registrations are temporarily disabled.',
      disable_landlord_applications: Boolean(body.disable_landlord_applications),
      disable_landlord_message:      String(body.disable_landlord_message ?? '').trim() || 'Landlord applications are temporarily closed.',
      announcement_active:           Boolean(body.announcement_active),
      announcement_text:             String(body.announcement_text ?? '').trim(),
      announcement_type:             ['info','warning','success','error'].includes(body.announcement_type) ? body.announcement_type : 'info',
