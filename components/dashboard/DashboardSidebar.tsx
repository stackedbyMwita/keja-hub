...
interface NavItem {
  ...
  exact?:   boolean  // match exact path only
  divider?: boolean
}
...
function isActive(pathname: string, item: NavItem): boolean {
  ...
  // For property reviews, also match sub-pages like /properties/[id] and /properties/[id]/images
  ...
  return (
    <aside className="hidden lg:flex flex-col w-60 min-h-screen border-r border-sidebar-border bg-sidebar shrink-0">
      ...
      <div className="h-16 flex items-center px-5 border-b border-sidebar-border shrink-0">
        <Link href="/" className="rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring hover:opacity-80 transition-opacity">
          <AppLogo />
        </Link>
      </div>
